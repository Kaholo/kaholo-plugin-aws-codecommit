...
function handleResult(result, query, keyField, valField) {
  ...
  const items = result.map(
    (item) => getAutoResult(keyField ? item[keyField] : item, keyField ? item[valField] : item),
  );
...
function listAuto(listFuncName, outputName, fields = []) {
  return async (query, pluginSettings, triggerParameters) => {
    ...
    const client = CodeCommitService.from(params, settings);
    let nextToken; const
      items = [];
    do {
...
async function listRegions(query, pluginSettings, actionParams) {
  ...
  let client;
  try {
    client = CodeCommitService.from(resolvedParams, settings);
  } catch (e) {
